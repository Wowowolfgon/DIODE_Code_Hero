...
void CAN_Chassis_SendCurrent2(void)
{
    ...
    data[0] = 0;
    data[1] = 0;			// 摩擦轮 左
    data[2] = (CAN_GM6020[1].set_voltage>>8)&0xff;//CAN_M3508[5].set_current >> 8;
    data[3] = (CAN_GM6020[1].set_voltage)&0xff;;		// 云台侧边电机			// 摩擦轮 右
    data[4] = CAN_M2006[0].set_current >> 8;
    data[5] = CAN_M2006[0].set_current;
    data[6] = CAN_M2006[1].set_current >> 8;
    data[7] = CAN_M2006[1].set_current;
...
void CAN_GM6020_Gimbal_SendVoltage()
{
    ...
    tx_data[2] = CAN_M3508[4].set_current >> 8;
    tx_data[3] = CAN_M3508[4].set_current;
    tx_data[4] = CAN_M3508[5].set_current >> 8;
    tx_data[5] = CAN_M3508[5].set_current;
    tx_data[6] = 0;
    tx_data[7] = 0;
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)  //接收回调函数
{
    ...
    if(hcan->Instance == CAN1)
    {
        ...
        if ( HAL_OK==HAL_RetVal)
        {
            ...
            /* M2006的反馈信息处理 */
            else if(RxMeg.StdId == CAN1_M2006_ID7||RxMeg.StdId == CAN1_M2006_ID8)	/* 0x207*/
            {
                CAN1_CNT ++;
				flag = RxMeg.StdId-CAN1_M2006_ID7;
                // 将数据保存在结构体中
                CAN_M2006[flag].angle=(rx_data[0]<<8)+rx_data[1];
                CAN_M2006[flag].speed=(rx_data[2]<<8)+rx_data[3];
                CAN_M2006[flag].current=(rx_data[4]<<8)+rx_data[5];
            }
			else if(RxMeg.StdId == CAN2_GM6020_ID2)
			{
...
//   	 get_total_angle_2006(&CAN_M2006[1]);
    }
...
//                    && (RxMeg.StdId <  CAN2_GM6020_ID1 + 7))
			...
			if(RxMeg.StdId >= CAN1_M3508_ID5 & RxMeg.StdId <= CAN1_M3508_ID6)	/* 0x201 ~ 0x204*/
            {
                CAN1_CNT ++;
                id = RxMeg.StdId-CAN1_M3508_ID1-1;
                // 将数据保存在结构体中
                CAN_M3508[id].angle=(rx_data[0]<<8)+rx_data[1];
                CAN_M3508[id].speed=(rx_data[2]<<8)+rx_data[3];
                CAN_M3508[id].current=(rx_data[4]<<8)+rx_data[5];
 ...
 //               LED_R_Toggle();		
	 ...
	 if(flag==1||flag==0)
	 get_total_angle_2006(&CAN_M2006[flag]);
	 if(flag==3)
...
void get_total_angle_2006(CAN_M2006_TypeDef *p) {
    ...
    p->total_angle += delta*0.001220703125;
    p->last_angle = p->angle;
}
